...
class HeuristicBot(BaseBot):
    ...
    def _choose_scored(self, label: str, context: BotContext, options: list[Any], rng: GameRng, scorer) -> BotDecision:
        if not options:
            return BotDecision(choice=None, reason=f"HeuristicBot had no legal options for {label}.", considered_options=[])
    ...
    def _score_card(self, context: BotContext, card_id: Any) -> float:
        ...
        if self._card_matches_secret(card, secret):
            score += 2.0 * self.config.skill + self._weight("secret_faction_card", 1.0)
    ...
    def _score_commit_option(self, context: BotContext, option: Any) -> float:
        ...
        if not card_ids:
            return 0.35 + (1 - self.config.risk_tolerance)
    ...
    def _reason(self, label: str, choice: Any, score: float) -> str:
        return (
            f"HeuristicBot chose {choice!r} for {label} with score {score:.2f}; "
            "prioritised secret faction, public leader pressure, card synergy, and controlled randomness."
        )
